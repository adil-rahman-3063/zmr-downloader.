...
async function runYtdlp(args, cookies) {
    let cookieFile;
    try {
...
app.get("/download", async (req, res) => {
    try {
        ...
        try {
            ...
            const audioOnlyFormat = formats.find((f) => f.acodec !== 'none' && f.acodec !== undefined &&
            ...
            const audioFormat = audioOnlyFormat ||
                ...
                formats.find((f) => f.acodec !== 'none' && !f.vcodec?.includes('vp'));
            if (!audioFormat) {
                console.error("✗ NO AUDIO FORMAT ERROR");
                console.error("Available formats sample:", formats.slice(0, 10).map((f) => ({
                    id: f.format_id,
                    acodec: f.acodec,
                    vcodec: f.vcodec,
                    ext: f.ext
